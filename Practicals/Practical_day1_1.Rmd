...
covariates or, as in this case, temporal effects . Here, we choose a second order
random walk model as prior distribution for the vector ${\mathbf f} =
(f_1, \dots,f_T)$, so that:
$$
\pi({\mathbf f}|\theta)\propto \theta^{(T-2)/2}\exp\left\{
-\frac{\theta}{2}\sum_{t=3}^T[f_t - 2f_{t-1} + f_{t-2}]^2
...
We want to compute
$$
\pi({\theta}|y) \propto \frac{\pi(y|x)\pi(x|\theta)}{\pi(x|y,\theta)}
$$
...
lprior_theta = function(theta,a=1,b=1)
  ???
